...
class PKRDG extends BaseController
{
    ...
    public function index()
    {
        if (session()->get('name') == True) {
            $data = [
                'title' => 'Page | Document PK-RDG',
                'user' => $this->User->getData(),
                'doc' => $this->Doc->getPKRDG(),
                'acdoc' => $this->Doc->getAcdoc(),
                'acreg' => $this->Acreg->getData()
            ];
            //dd($data);
            return view('doconboard/pkrdg/V_rdg', $data);
        } else {
            return redirect()->to('/');
        }
    }
    public function insert()
    {
        ...
        //dd($data);
        return redirect()->to('/PK-RDG');
    }
    public function getPdf($id_document)
    {
        if (session()->get('name') == True) {
            $data = [
                'doc' => $this->Doc->getDetailPKRDG($id_document),
                'user' => $this->User->getData(),
            ];

            return view('doconboard/pkrdg/pdf', $data);
        } else {
            return redirect()->to('/');
        }
    }
    public function update($id_document)
    {
        if (session()->get('name') == True) {
            $data = [
                'title' => 'Page Update | PK-RDG',
                'doc' => $this->Doc->getDetailPKRDG($id_document),
                'user' => $this->User->getData(),
                'acdoc' => $this->Doc->getAcdoc()
            ];

            return view('doconboard/pkrdg/V_update', $data);
        } else {
            return redirect()->to('/');
        }
    }
    ...
    public function Convert()
    {
        if (session()->get('name') == True) {
            ...
            //dd($data);
            //return 
            $html = view('doconboard/pkrdg/V_Pdf', $data);
            ...
            $dompdf->render();
            $dompdf->stream('REPORT DOCUMENT ONBOARD PK-RDG ' . date('F Y') . '.pdf');
        } else {
            return redirect()->to('/');
        }
    }
    ...
    public function Print()
    {
        if (session()->get('name') == True) {
            ...
            //dd($data);
            return view('doconboard/pkrdg/Print', $data);
        } else {
            return redirect()->to('/');
        }
    }
