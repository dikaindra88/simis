...
class NewCondition extends BaseController
{
    public function __construct()
    {
        $this->Option = new Options();
        $this->User = new UsersModel();
        $this->New = new SparepartModel();
        helper('form');
    }
    public function index()
    {
        ...
        $data = [
            'title' => 'Page | Condition NEW',
            'new' => $this->New->getNew(),
            'user' => $this->User->getData(),
            'first_date'=> '',
            'end_date'=> ''
        ];
        return view('sparecondition/new/V_new', $data);
    }else{
        return redirect()->to('/');
    }
    }
    ...
    public function show()
    {
        ...
        $data = [
            'title' => 'Page | Condition NEW',
        'new' => $this->New->getDetailNew($first_date, $end_date),
        'user' => $this->User->getData(),
    ...
    public function excel($first_date, $end_date, $mid_date, $first_date1, $end_date1, $mid_date1)
    {
        ...
        $data = [
            'new' => $this->New->getDetailNew($first, $end),
            'first_date'=> $first,
            'end_date'=> $end

        ];
         //dd($data);
        return view('sparecondition/new/excel', $data);
    }else{
        return redirect()->to('/');
    }
    }
    public function print($first_date, $end_date, $mid_date, $first_date1, $end_date1, $mid_date1)
    {
        ...
        //dd($data);
       //return 
        $html = view('sparecondition/new/print', $data);
        ...
        $dompdf->render();
        $dompdf->stream('Report Data List New ' . date('F Y') . '.pdf');
    }else{
        return redirect()->to('/');
    }
}
